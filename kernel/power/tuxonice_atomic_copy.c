...
int __toi_post_context_save(void)
{
	...
	if (extra_pd1_pages_used > extra_pd1_pages_allowance) {
		pr_warn("Pageset1 has grown by %lu pages. extra_pages_allowance is currently only %lu.\n",
		...
		if (try_allocate_extra_memory()) {
			pr_warn("Failed to allocate the extra memory needed. Restarting the process.");
			set_abort_result(TOI_EXTRA_PAGES_ALLOW_TOO_SMALL);
			return 1;
		}
		pr_warn("Looks like there's enough free ram and storage to handle this, so continuing anyway.");
		/*
...
Failed:
	...
	pr_warn("[%s] 0x%p 0x%p 0x%p\n", __func__,
		restore_highmem_pblist->address, restore_highmem_pblist->orig_address,
		restore_highmem_pblist->next);
