...
static int toi_file_register_storage(void)
{
	...
	if (!strlen(toi_file_target)) {
		toi_message(TOI_IO, TOI_VERBOSE, 0, "Register file storage: No target filename set.");
		return 0;
	}
	...
	if (IS_ERR(target_file) || !target_file) {
		...
		if (!toi_file_dev_t) {
			...
			pr_warn("Open file %s returned %p and name_to_devt failed.\n", toi_file_target, target_file);
			if (error) {
				pr_warn("Stating the file also failed. Nothing more we can do.");
				return 0;
		...
		if (IS_ERR(toi_file_target_bdev)) {
			pr_warn("Got a dev_num (%lx) but failed to open it.\n", (unsigned long)toi_file_dev_t);
	...
	if (S_ISLNK(target_inode->i_mode) || S_ISDIR(target_inode->i_mode) ||
		...
		pr_warn("File support works with regular files, character files and block devices.");
	...
	if (!used_devt) {
		...
		if (!toi_file_target_bdev) {
			pr_warn("%s is not a valid file allocator target.", toi_file_target);
			return 0;
		}
	...
	if (!devinfo) {
		pr_warn("Failed to allocate a toi_bdev_info struct for the file allocator.\n");
		return -ENOMEM;
	}
	 ...
	 */
	if (result)
		pr_debug("Failed to get fs_info for file device (%d).\n", result);
	...
	file_chain = devinfo;
	toi_message(TOI_IO, TOI_VERBOSE, 0, "Dev_t is %lx. Prio is %d. Bmap shift is %d. Blocks per page %d.",
	...
	toi_message(TOI_IO, TOI_VERBOSE, 0, "Registering file storage, %lu pages.", pages_available);
...
static int toi_file_allocate_storage(struct toi_bdev_info *chain, unsigned long request)
{
	...
	toi_message(TOI_IO, TOI_VERBOSE, 0, "Pages available: %lu. Allocated: %lu. Allocating: %lu from file",
 ...
 *
 * Test whether the target file is valid for hibernating.
 **/
