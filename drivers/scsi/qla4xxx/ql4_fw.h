...
struct ql_iscsi_stats {
	uint64_t mac_tx_frames; /* 0000每0007 */
	uint64_t mac_tx_bytes; /* 0008每000F */
	uint64_t mac_tx_multicast_frames; /* 0010每0017 */
	uint64_t mac_tx_broadcast_frames; /* 0018每001F */
	uint64_t mac_tx_pause_frames; /* 0020每0027 */
	uint64_t mac_tx_control_frames; /* 0028每002F */
	uint64_t mac_tx_deferral; /* 0030每0037 */
	uint64_t mac_tx_excess_deferral; /* 0038每003F */
	uint64_t mac_tx_late_collision; /* 0040每0047 */
	uint64_t mac_tx_abort; /* 0048每004F */
	uint64_t mac_tx_single_collision; /* 0050每0057 */
	uint64_t mac_tx_multiple_collision; /* 0058每005F */
	uint64_t mac_tx_collision; /* 0060每0067 */
	uint64_t mac_tx_frames_dropped; /* 0068每006F */
	uint64_t mac_tx_jumbo_frames; /* 0070每0077 */
	uint64_t mac_rx_frames; /* 0078每007F */
	uint64_t mac_rx_bytes; /* 0080每0087 */
	uint64_t mac_rx_unknown_control_frames; /* 0088每008F */
	uint64_t mac_rx_pause_frames; /* 0090每0097 */
	uint64_t mac_rx_control_frames; /* 0098每009F */
	uint64_t mac_rx_dribble; /* 00A0每00A7 */
	uint64_t mac_rx_frame_length_error; /* 00A8每00AF */
	uint64_t mac_rx_jabber; /* 00B0每00B7 */
	uint64_t mac_rx_carrier_sense_error; /* 00B8每00BF */
	uint64_t mac_rx_frame_discarded; /* 00C0每00C7 */
	uint64_t mac_rx_frames_dropped; /* 00C8每00CF */
	uint64_t mac_crc_error; /* 00D0每00D7 */
	uint64_t mac_encoding_error; /* 00D8每00DF */
	uint64_t mac_rx_length_error_large; /* 00E0每00E7 */
	uint64_t mac_rx_length_error_small; /* 00E8每00EF */
	uint64_t mac_rx_multicast_frames; /* 00F0每00F7 */
	uint64_t mac_rx_broadcast_frames; /* 00F8每00FF */
	uint64_t ip_tx_packets; /* 0100每0107 */
	uint64_t ip_tx_bytes; /* 0108每010F */
	uint64_t ip_tx_fragments; /* 0110每0117 */
	uint64_t ip_rx_packets; /* 0118每011F */
	uint64_t ip_rx_bytes; /* 0120每0127 */
	uint64_t ip_rx_fragments; /* 0128每012F */
	uint64_t ip_datagram_reassembly; /* 0130每0137 */
	uint64_t ip_invalid_address_error; /* 0138每013F */
	uint64_t ip_error_packets; /* 0140每0147 */
	uint64_t ip_fragrx_overlap; /* 0148每014F */
	uint64_t ip_fragrx_outoforder; /* 0150每0157 */
	uint64_t ip_datagram_reassembly_timeout; /* 0158每015F */
	uint64_t ipv6_tx_packets; /* 0160每0167 */
	uint64_t ipv6_tx_bytes; /* 0168每016F */
	uint64_t ipv6_tx_fragments; /* 0170每0177 */
	uint64_t ipv6_rx_packets; /* 0178每017F */
	uint64_t ipv6_rx_bytes; /* 0180每0187 */
	uint64_t ipv6_rx_fragments; /* 0188每018F */
	uint64_t ipv6_datagram_reassembly; /* 0190每0197 */
	uint64_t ipv6_invalid_address_error; /* 0198每019F */
	uint64_t ipv6_error_packets; /* 01A0每01A7 */
	uint64_t ipv6_fragrx_overlap; /* 01A8每01AF */
	uint64_t ipv6_fragrx_outoforder; /* 01B0每01B7 */
	uint64_t ipv6_datagram_reassembly_timeout; /* 01B8每01BF */
	uint64_t tcp_tx_segments; /* 01C0每01C7 */
	uint64_t tcp_tx_bytes; /* 01C8每01CF */
	uint64_t tcp_rx_segments; /* 01D0每01D7 */
	uint64_t tcp_rx_byte; /* 01D8每01DF */
	uint64_t tcp_duplicate_ack_retx; /* 01E0每01E7 */
	uint64_t tcp_retx_timer_expired; /* 01E8每01EF */
	uint64_t tcp_rx_duplicate_ack; /* 01F0每01F7 */
	uint64_t tcp_rx_pure_ackr; /* 01F8每01FF */
	uint64_t tcp_tx_delayed_ack; /* 0200每0207 */
	uint64_t tcp_tx_pure_ack; /* 0208每020F */
	uint64_t tcp_rx_segment_error; /* 0210每0217 */
	uint64_t tcp_rx_segment_outoforder; /* 0218每021F */
	uint64_t tcp_rx_window_probe; /* 0220每0227 */
	uint64_t tcp_rx_window_update; /* 0228每022F */
	uint64_t tcp_tx_window_probe_persist; /* 0230每0237 */
	uint64_t ecc_error_correction; /* 0238每023F */
	...
	uint32_t hdr_digest_err; /* 02B0每02B3 */
	uint32_t data_digest_err; /* 02B4每02B7 */
	uint32_t conn_timeout_err; /* 02B8每02BB */
	uint32_t framing_err; /* 02BC每02BF */

	uint32_t tx_nopout_pdus; /* 02C0每02C3 */
	uint32_t tx_scsi_cmd_pdus;  /* 02C4每02C7 */
	uint32_t tx_tmf_cmd_pdus; /* 02C8每02CB */
	uint32_t tx_login_cmd_pdus; /* 02CC每02CF */
	uint32_t tx_text_cmd_pdus; /* 02D0每02D3 */
	uint32_t tx_scsi_write_pdus; /* 02D4每02D7 */
	uint32_t tx_logout_cmd_pdus; /* 02D8每02DB */
	uint32_t tx_snack_req_pdus; /* 02DC每02DF */

	uint32_t rx_nopin_pdus; /* 02E0每02E3 */
	uint32_t rx_scsi_resp_pdus; /* 02E4每02E7 */
	uint32_t rx_tmf_resp_pdus; /* 02E8每02EB */
	uint32_t rx_login_resp_pdus; /* 02EC每02EF */
	uint32_t rx_text_resp_pdus; /* 02F0每02F3 */
	uint32_t rx_scsi_read_pdus; /* 02F4每02F7 */
	uint32_t rx_logout_resp_pdus; /* 02F8每02FB */

	uint32_t rx_r2t_pdus; /* 02FC每02FF */
	uint32_t rx_async_pdus; /* 0300每0303 */
	uint32_t rx_reject_pdus; /* 0304每0307 */
