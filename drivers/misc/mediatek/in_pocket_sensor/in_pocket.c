...
static struct inpk_context *inpk_context_obj;
		...
		if (false == cxt->is_active_data) {
			...
			if (err) {
				...
				if (err) {
					...
					if (err) {
						INPK_ERR
						    ("enable_in_pocket enable(%d) err 3 timers = %d\n",
						     enable, err);
						return err;
					}
				}
...
static ssize_t inpk_show_enable_nodata(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct inpk_context *cxt = NULL;
...
/*----------------------------------------------------------------------------*/
static ssize_t inpk_show_active(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct inpk_context *cxt = NULL;
...
static ssize_t inpk_store_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t inpk_show_delay(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t inpk_show_batch(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t inpk_show_flush(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
...
static ssize_t inpk_show_devnum(struct device *dev, struct device_attribute *attr, char *buf)
{
	char *devname = NULL;
...
/* cxt->inpk_ctl.enable_nodata = ctl->enable_nodata; */
	...
	err = sysfs_create_group(&inpk_context_obj->mdev.this_device->kobj, &inpk_attribute_group);
	if (err < 0) {
...
static int inpk_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&inpk_context_obj->idev->dev.kobj, &inpk_attribute_group);

	err = misc_deregister(&inpk_context_obj->mdev);
	if (err)
