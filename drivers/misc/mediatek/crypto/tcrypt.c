			    ...
			    unsigned int aad_size, u8 *keysize)
{
	...
	if (aad_size >= PAGE_SIZE) {
		pr_cont("associate data length (%u) too big\n", aad_size);
		return;
	}
	...
	if (IS_ERR(tfm)) {
		pr_cont("alg: aead: Failed to load transform for %s: %ld\n", algo,
			PTR_ERR(tfm));
		goto out_notfm;
	}
	...
	if (!req) {
		pr_cont("alg: aead: Failed to allocate request for %s\n",
			algo);
		goto out_noreq;
	}
	...
	do {
		...
		do {
			...
			if (ret) {
				pr_cont("%s() failed return code=%d\n", e, ret);
				break;
			}
			b_size++;
			i++;
				...
				struct hash_speed *speed)
{
	...
	if (!req) {
		pr_cont("ahash request allocation failure\n");
		goto out;
	}
				...
				unsigned int tcount, u8 *keysize)
{
	...
	if (!req) {
		pr_cont("tcrypt: skcipher: Failed to allocate request for %s\n",
			algo);
		goto out;
	}
